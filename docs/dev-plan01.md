...
このプロジェクトはテストを RSpec（`spec/`）で書いており、GitHub Actions のワークフローは Rails が生成した Minitest 向け（`bin/rails test`）のまま残っている可能性がある。またトリガーが `master` のみだが、リポジトリのメインブランチは `main` を使う。
...
- GitHub のリポジトリ設定でデフォルトブランチが `main` になっていることと、`ci.yml` の `push.branches` が一致していることを確認する。`master` のままだと `main` へ直 push したときにテストジョブが走らない。
...
## ローカル検証用: DB 同梱の Docker Compose—作業手順の概要

### 位置づけ（先に決める）

- この Docker Compose は、本番デプロイや Kamal の代替ではなく、手元で動作や DB 付きの流れを試すためのローカル検証用として用意する。
- ホスト OS で直接 Ruby を動かす開発手順（既存の README）とは別線。必要な人だけが使えばよい。

目的: 手元で PostgreSQL とアプリをまとめて起動し、`docker compose up` 程度でブログを試せるようにする。Kamal 用の本番 `Dockerfile` とは役割を分ける。

### 方針の決め

- 既存のルート `Dockerfile` は本番向け（`RAILS_ENV=production`、Thruster、jemalloc 等）。Compose ではそのイメージを流用するか、開発向けに別 Dockerfile（またはターゲット）を置いて `rails server` で動かすかを決める。後者はホットリロードや `development` 用 Gem を使いやすい。
- ルートの `Dockerfile` に `COPY vendor/*` などがあり、リポジトリに `vendor/` が無いとビルドが失敗する可能性がある。Compose 実装時に `docker build` が通るか先に確認し、必要なら Dockerfile の調整を別タスクで行う。

### 手順（実装）

1. `compose.yaml`（または `docker-compose.yml`）を追加する  
    - `services` に少なくとも PostgreSQL（公式 `postgres` イメージ、バージョンはプロジェクトに合わせる）を定義する。  
    - アプリ用サービスを定義し、`build` で `Dockerfile` を参照するか、`image` を使うか決める。  
    - Compose 内の DB ホスト名（例: `db`）で名前解決できるよう、`DATABASE_URL` を `postgres://ユーザー:パスワード@db:5432/データベース名` の形でアプリに渡す。

2. アプリ側の環境変数を整理する  
    - `RAILS_MASTER_KEY`: `config/master.key` の内容と同じ値を、コンテナに渡す（ファイルをマウントせず env で渡す運用が一般的）。秘密は Git に含めない。  
    - 本番イメージのままなら `RAILS_ENV=production` が既定のことがある。開発用 Rails で試すなら `RAILS_ENV=development` とし、`database.yml` の接続と整合させる。

3. DB ユーザーとデータベース名を揃える  
    - Postgres サービスの `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB` を、`DATABASE_URL` および既存の `config/database.yml`（`ruby_blog` ユーザー等）と矛盾しないようにする。既存のローカル手順と二重管理にならないよう、Compose 専用のユーザー名にするか、README に対応表を書く。

4. 秘密・環境ファイルのテンプレート  
    - リポジトリには `.env.docker.example` のような名前で、必要なキー名だけ列挙したテンプレートを置く（値はダミー）。実際の `.env.docker` は `.gitignore` する。

5. ポートとヘルスチェック  
    - アプリを `3000` で試すなら `ports: ["3000:3000"]` など。本番 Dockerfile は 80 を EXPOSE しているため、マッピングは `3000:80` になる場合がある。Thruster 経由か直接 Puma かで確認する。

6. 起動とマイグレーション  
    - 本番 `Dockerfile` の `ENTRYPOINT` はサーバ起動時に `db:prepare` を呼ぶ想定がある（`bin/docker-entrypoint`）。開発用別定義なら `docker compose run web bin/rails db:prepare` を README に書く。

7. README に短い節を追加する  
    - 前提（Docker / Compose のインストール）、このスタックがローカル検証用であること、コピーする環境ファイル、`docker compose up`、アクセス URL、トラブル時（ビルド失敗・DB 接続）の一言。

### 補足

- この Compose は本番デプロイの代替ではない。Kamal・本番 `Dockerfile` の運用とは README・ドキュメント上も切り分ける。
- 検証用でも `RAILS_MASTER_KEY` や DB パスワードは秘密として扱う（コミットしない）。
